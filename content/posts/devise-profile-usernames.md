...
- Goal is to have a URL for a User's profile page as: `/users/:username`
- I want to generate a unique `username` and not request on the sign-up form

**name_of_person** - We will use the [name_of_person](https://github.com/basecamp/name_of_person) gem by Basecamp. This gem created a pseudo-field for full name (requires `first_name` and `last_name` in the `User` table). It has many other abstractions, but this is the only feature we will use.
...
## Basic set up
We need to set up a basic app using the defaults is just fine: `rails new awesome_app`. Next we will go ahead and set up a static controller for a home route: `rails g controller static home`.
...
```
We are going to generate a devise `User` model, which is a little different: `rails generate devise User`.

We have to add two columns to the migration that was generated by Devise for `name_of_person`. Add `first_name` and `last_name` somewhere within the database migration:
``` 
...
```
Then migrate with `rails db:migrate`. Devise will automatically add the appropriate routes, but it always a good idea to check. so make sure that in `config/routes.rb` the route is found: `devise_for: users`. 

Add to the `User` model to use the `name_of_person` gem, adding `has_person_name`:
```ruby
...
```
We will need to add the `name` field to the sign-in and sign-up forms, so we need to generate the views: `rails generate devise:views`. In `app/views/devise/registrations/new.html.erb` and `.../sessions/new.html.erb` add the following for the name field:
``` 
<h2>Sign up</h2>
...
```
If you already have `Users` created, from the `rails console` execute: `User.find_each(&:save)`
